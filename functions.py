...
data_pt=dt.pt_data
 ...
 # -- (1) Buy trade Count -- #
def buy_trade_count_PT(pt_data: dict = None) -> True:
    """
    This function finds 
    ...
    ob_m1 : 
    """
    pt_ts = list(pt_data.keys())
    l_ts = [pd.to_datetime(i_ts) for i_ts in pt_ts]
    pt_m1 = pd.DataFrame(pt_data[pt_data["side"]=='buy']['side'].resample('30s').count())
    return pt_m1

# -- (2) Sell trade count -- # 
def sell_trade_count_PT(data_PT: dict = None) -> True:
    """
    This function finds 
    ...
    """
    pt_ts = list(pt_data.keys())
    pt_m2 = pt_data[pt_datat["side"]=='sell']['side'].resample(t).count()
    return pt_m2
...
def sell_trade_count_PT(data_PT: dict = None) -> True:
    ...
    """
    pt_ts = list(pt_data.keys())
    pt_m3 = pt_data['side'].resample(t).count() 
    return pt_m3

# -- (4) Difference in trade count (Buy - Sell) -- #
def sell_trade_count_PT(pt_data: dict = None) -> True:
    """
    This function finds 
